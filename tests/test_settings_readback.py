...
class SettingsReadbackTests(unittest.TestCase):
    ...
    def test_blocked_readback_cannot_be_relabelled_conformant_without_evidence(self):
        ...
        with self.assertRaisesRegex(
            settings_readback.SettingsReadbackError,
            "blocked organization readback cannot be relabelled conformant",
        ):
            settings_readback.validate_settings_readback(normalized, ledger)

    def test_blocked_readback_issue_must_match_normalized_evidence(self):
        normalized = self.normalized()
        ledger = self.ledger()
        row = next(row for row in ledger["rows"] if row["id"] == "ORG-READBACK-BLOCKED-001")
        row["remediation_issue"] = "https://github.com/grandchallenge/.github/issues/999999"
        with self.assertRaisesRegex(
            settings_readback.SettingsReadbackError,
            "blocked organization readback remediation issue drift",
        ):
            settings_readback.validate_settings_readback(normalized, ledger)
